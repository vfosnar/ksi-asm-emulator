...
# Generate instructions dictionary

output_instructions = {}


for i, (instruction, table_params) in enumerate(OPCODES):
    output_instructions[instruction] = output_instructions.get(instruction, [])

    instr_info = {
        "opcode": i,
    }

    output_instructions[instruction].append((table_params, instr_info))

    # Co tam chci přidávat:
    # 1) parametry
    # 2) opcode
    # 3) info

    # TODO: Nakonec seřadit od nejkratší po nejdelší

    # instructions[instruction].append(i)


grp_opcodes = {
    "GRP1": [('Eb Ib', 0x80), ('Ev Iv', 0x81), ('Eb Ib', 0x82), ('Ev Ib', 0x83)],
    "GRP2": [('Eb 1', 0xD0), ('Ev 1', 0xD1), ('Eb CL', 0xD0), ('Ev CL', 0xD1)],
    "GRP3a": [('Eb', 0xF6)],
    "GRP3b": [('Ev', 0xF7)],
    "GRP4": [('Eb', 0xFE)],
    "GRP5": [('Ev', 0xFF)]
...
# Udělat každý s každým
for grp, instructions in GRPs.items():
    for grp_params, opcode in grp_opcodes[grp]:
        for i, (instruction, table_params) in enumerate(instructions):
            output_instructions[instruction] = output_instructions.get(
                instruction, [])
            ...
            if table_params:
                grp_params = table_params

            output_instructions[instruction].append((grp_params, instr_info))

            # Co tam chci přidávat:
            # 1) parametry
            # 2) --opcode-- (součást info)
            # 3) info
...
INSTRUCTIONS = {
    ...
    "CALL": [
        ("Ap", {"opcode": 154}),
        ("Jv", {"opcode": 232}),
        ("Ev", {"opcode": 255, "modrm": 16}),
        ("Mp", {"opcode": 255, "modrm": 24}),
    ],
    "WAIT": [("", {"opcode": 155})],
    "PUSHF": [("", {"opcode": 156})],
    "POPF": [("", {"opcode": 157})],
    ...
    "SCASB": [("", {"opcode": 174})],
    "SCASW": [("", {"opcode": 175})],
    "RET": [("Iw", {"opcode": 194}), ("", {"opcode": 195})],
    "LES": [("Gv Mp", {"opcode": 196})],
    "LDS": [("Gv Mp", {"opcode": 197})],
    "RETF": [("Iw", {"opcode": 202}), ("", {"opcode": 203})],
    "INT": [("3", {"opcode": 204}), ("Ib", {"opcode": 205})],
    "INTO": [("", {"opcode": 206})],
    "IRET": [("", {"opcode": 207})],
    "AAM": [("I0", {"opcode": 212})],
    "AAD": [("I0", {"opcode": 213})],
    "XLAT": [("", {"opcode": 215})],
    "CO-PROCESSOR INSTRUCTIONS": [("", {"opcode": 216})],
    "LOOPNZ": [("Jb", {"opcode": 224})],
    "LOOPZ": [("Jb", {"opcode": 225})],
    "LOOP": [("Jb", {"opcode": 226})],
    "JCXZ": [("Jb", {"opcode": 227})],
    "IN": [
    ...
    "NOT": [
        ("Eb Ib", {"opcode": 246, "modrm": 16}),
        ("Ev Iv", {"opcode": 247, "modrm": 16}),
    ],
    "NEG": [
        ("Eb Ib", {"opcode": 246, "modrm": 24}),
        ("Ev Iv", {"opcode": 247, "modrm": 24}),
    ],
    "MUL": [
        ("Eb Ib", {"opcode": 246, "modrm": 32}),
        ("Ev Iv", {"opcode": 247, "modrm": 32}),
    ],
    "IMUL": [
        ("Eb Ib", {"opcode": 246, "modrm": 40}),
        ("Ev Iv", {"opcode": 247, "modrm": 40}),
    ],
    "DIV": [
        ("Eb Ib", {"opcode": 246, "modrm": 48}),
        ("Ev Iv", {"opcode": 247, "modrm": 48}),
    ],
    "IDIV": [
        ("Eb Ib", {"opcode": 246, "modrm": 56}),
        ("Ev Iv", {"opcode": 247, "modrm": 56}),
    ],
}
...
def get_size_from_params(params: str):
    if params in ["", " ", "3", "I0"]:
        # Actually, i don't know what's IO, but i don't need it for KSI
        return 0
    if "b" in params:
        return 8
    if "w" in params or "v" in params:
        return 16
    if "p" in params:
        return 32

    for param in params.split(" "):
        if param in ["AL", "BL", "CL", "DL", "AH", "BH", "CH", "DH"]:
            return 8
        if param in ["AX", "BX", "CX", "DX", "SI", "DI", "BP", "SP", "ES", "CS", "SS", "DS"]:
            return 16

    assert False, f"Unknown size for {params}"


# for instruction, params in INSTRUCTIONS.items():
#     for param, info in params:
#         size = get_size_from_params(param)

#         new_instruction = f"{instruction}{size}"
#         UPDATED_INSTRUCTIONS[new_instruction] = UPDATED_INSTRUCTIONS.get(
#             new_instruction, [])

#         UPDATED_INSTRUCTIONS[new_instruction].append((param, info))


print(INSTRUCTIONS_v2)
